...
다음 포스트에서는 `HTTP Authorization Header`를 이용하여 쿠버네티스의 인증을 처리하는 방법에 대해서 살펴보도록 하겠습니다.
