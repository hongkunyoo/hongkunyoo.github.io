...
1. X.509 Client Certs: 인증서를 이용하여 클라이언트 인증을 하는 방법에 대해서 살펴 봅니다.
2. Static file: basic, token auth 파일을 이용한 HTTP Authorization 방법에 대해서 살펴 봅니다. (TBA)
3. Service Account: 쿠버네티스에서 제공해주는 Service Account라는 리소스를 이용한 인증 방법에 대해서 살펴 봅니다. (TBA)
4. OpenID Connect: Google OAuth와 같은 3rd party 인증 provider를 이용하여 인증하는 방법에 대해서 살펴 봅니다. (TBA)
5. Webhook: Webhook을 통해 쿠버네티스 내부 인증 시스템이 아닌 외부 인증 메커니즘을 이용하는 방법에 대해서 살펴 봅니다. (TBA)
6. Proxy Auth: API server에 도달하기 이전에 먼저 proxy 인증 서버를 거쳐 미리 인증을 받아 넘어가는 방법에 대해서 살펴 봅니다. (TBA)
...
쿠버네티스 인증 완벽이해 시리즈에서는 Authentication에 대해서 다룹니다.
...
특이하게 쿠버네티스에서는 내부적으로 유저 인증 정보를 저장하지 않습니다. 대부분의 웹 서비스나 인증 서버들은 사용자 정보를 내부적으로 저장하여 사용자로부터 인증 정보를 전달 받았을 때 저장된 정보를 바탕으로 인증을 처리합니다.
쿠버네티스는 이와 다르게 따로 인증 정보를 저장하지 않고 각각의 인증 시스템에서 제공해주는 기능들을 활용하여 사용자 인증을 하고 유저 인식 (identify)을 합니다. 처음에는 이러한 쿠버네티스의 방식에 대해서 의아해 했지만 이러한 특징 때문에 쿠버네티스에서는 인증 체계를 확장하는 것이 굉장히 편리해 집니다. 왜냐하면 쿠버네티스 내부 인증체계의 종속성이 걸리는 부분이 거의 없어지기 때문입니다.
...
(어찌보면 당연한 얘기 같습니다. api 서버를 생성하기 위해서 api 서버에 요청을 할 수 없으니 Static Pod를 통해서 api 서버를 생성하는 것이죠.)

#### 4. API 서버는 그냥 웹서버

API 서버는 굉장히 복잡해 보이지만 사실 멀리서 겉모습만 바라보면 일반적인 웹서버와 다를 바가 없습니다. REST API로 요청을 보내면 json, yaml 형식으로 결과를 리턴해주며 내부적으로 DB에 데이터를 저장하는 웹 서버입니다. 그렇기 때문에 쿠버네티스의 사용자 인증 방식도 평범한 웹 서버의 인증 방식과 크게 다르지 않습니다. TLS를 이용한 서버 인증, HTTP Authorization Header를 통한 인증, OAuth 인증, Webhook 및 proxy 인증 서버를 통한 인증이 그것이죠. 저도 복잡하고 거대한 쿠버네티스의 인증 체계를 공부한다기 보다는 작고 간단한 웹서버 인증 방법에 대해서 살펴본다는 생각으로 공부하였습니다.

그럼 쿠버네티스 authentication 첫번째 방법, X.509 Client Certification에 대해서 알아보도록 합시다.

## X.509 Client Certificate

### X.509

X.509 기술은 비대칭 암호화 기술을 이용한 공개키 기반의 인증 체계입니다. X.509 기술을 이용하여 암호화된 통신(HTTPS)을 하거나 인증(certificate)에 사용할 수도 있습니다. 쿠버네티스에서는 X.509 Certificate를 이용하여 사용자의 신원을 **인증**하는 목적으로 사용하고 있습니다.
X.509의 기술에 대해서 간략하게 키워드 중심으로 정리해 보겠습니다.


#### Public Private Keys

X.509 기술의 근간이 되는 암호화 기술입니다. 여기에는 Public 키와 Private 키가 존재합니다. Public 키는 누구나 가질 수 있으며 Private 키는 오직 Public, Private 키페어 소유자만 가지고 있습니다. Public 키를 이용하여 메세지를 암호화하게 되면 오직 해당 Public키에 매핑되는 Private 키로만 복호화가 가능합니다. 반대로 Priate 키를 이용하여 메세지를 암호화할 수 있는데요, 이때 누구나 Public 키를 이용하여 암호화된 메세지에 대해서 정말 Private 키의 소유자가 작성한 메세지인지를 검증할 수 있습니다. Private 키로 메세지를 암호화하는 것을 *디지털 서명*(digital signature)이라고 합니다. X.509 Client Certificate에서는 이러한 방식을 이용하여 사용자의 신원을 확인합니다.

![](/assets/images/k8s-auth/01-03.png)

#### Certificate

Public 키는 단순히 메세지를 암호화하고 Private 키로 암호화된 메세지를 검증하는 키에 불과합니다. 인증 체계를 구축하기 위해서는 인증서 발급 기관, 인증서 유효기간, 인증 신원, 서명 알고리즘 등과 같이 다양한 정보들이 필요합니다. Certificate은 Public 키를 포함한 다양한 인증 정보를 담고 있는 문서입니다. X.509 기술을 이용할 때 직접 Public 키를 사용하기 보단 보통 Certificate (인증서)을 활용합니다.

#### Certificate Authority

인증서에는 해당 인증서를 발급해준 발급 기관 (Issuer) 정보를 포함하고 있습니다. 해당 정보를 이용하여 지금 보고 있는 인증서가 아무에게서나 발급된 인증서가 아니라 공인된 인증 기관에서 발급된 인증서라는 것을 확인할 수 있습니다. 이것을 Certificate Authority (인증 기관)이라 부르고 짧게 CA라고도 합니다. 이 CA 또한 인증서로 이루어져 있어 해당 Public 키로 인증기관의 유효성을 동일한 메커니즘으로 검사할 수 있습니다. CA도 마찬가지로 CA의 인증서를 발급한 인증 기관(CA)이 존재하며 이것을 Certificate Chain (인증 체인)이라고 하고 인증 체인의 가장 끝에는 `Root CA`라고 하는 인증서가 있습니다. 이 Root CA는 따로 인증 기관이 존재하지 않으며 스스로가 스스로를 인증합니다.


## Certificate을 이용한 사용자 인증

지금까지 X.509 Certificate에 대해 알아 봤는데 이 기술을 이용하여 어떻게 사용자 인증을 할 수 있는지 살펴 보겠습니다.

### 1. 인증서 발급

![](/assets/images/k8s-auth/01-04.png)

먼저 테스트에 사용할 MyRootCA라는 이름을 가지는 Root CA를 만듭니다. MyRootCA를 통하여 서버 인증에 사용할 server-side 인증서 key pair와 사용자 인증에 사용할 client-side 인증서 key pair를 생성합니다.

- Root CA Certificate
  - key pair: rootCA.pem, rootCA-key.pem
  - 클라이언트와 서버 인증서를 발급하는 주체, 클라이언트와 서버는 서로의 인증서가 Root CA가 발급한 공인된 인증서임을 확인할 수 있습니다.
- Server Certificate
  - key pair: server.pem, server-key.pem
  - 서버의 신원을 확인하는 인증서와 서버 자체 key
- Client Certificate
  - key pair: client.pem, client-key.pem
  - 클라이언트의 신원을 확인하는 인증서와 클라이언트 자체 key


#### 1) 인증서 생성툴 설치

사용자 인증서를 직접 생성하지 않고 인증서 생성툴을 이용하여 편하게 생성할 수 있습니다. `easyrsa`, `openssl`, `cfssl` 등과 같이 다양한 툴들이 존재하는데 여기서는 Cloudflare에서 만든 `cfssl`을 이용하여 생성하겠습니다.
...
인증서는 CSR (인증서 서명 요청)는 문서를 통하여 실제 인증서가 생성됩니다. 간단하게 인증서를 생성하기 위한 요청 문서라고 생각하시면 됩니다. 더 자세한 내용은 아래 문서들을 참고하시기 바랍니다.

- https://en.wikipedia.org/wiki/Certificate_signing_request
- https://www.sslshopper.com/what-is-a-csr-certificate-signing-request.html

*참고: 실제 CSR문서는 `-----BEGIN CERTIFICATE REQUEST-----` 로 시작되는 문서이지만 `cfssl` 툴을 이용하는 경우 자체적은 json 형식을 이용하여 인증서를 생성합니다.*
...
이제 인증에 필요한 인증서 발급이 완료되었습니다. 먼저 서버 인증서를 등록해 보도록 하겠습니다. 아래와 같이 server-side 인증서를 nginx 설정파일에 적용합니다.
...
sudo cp /etc/kubernetes/pki/ca.key ~/auth/k8s-rootCA-key.pem
...
nginx 설정 파일들도 앞에 `k8s-` prefix를 붙여 줍니다.
...
위와 같은 방법으로도 정상적으로 nginx 서버에 연결이 되는 것을 확인할 수 있습니다. 이를 미루어 볼 때, 다음과 같이 해석할 수 있습니다.

#### root CA
쿠버네티스에서 사용하는 rootCA의 키들은 다음과 같습니다.
- 인증서: /etc/kubernetes/pki/ca.crt
...
- 인증서: /etc/kubernetes/admin.conf `client-certificate-data`
- key: /etc/kubernetes/admin.conf `client-key-data`
...
# 신규 사용자 인증서로 대체
kubectl config --kubeconfig=$HOME/kubeconfig set-credentials kubernetes-admin --client-certificate=k8s-new-client.pem --client-key=k8s-new-client-key.pem
...
# curl - 신규 사용자 인증
API_SERVER_ADDR=localhost  # 예시
API_SERVER_PORT=6443       # 예시
...
이번 포스트에서 X.509 기술에 대한 전반적인 내용에 대해서 다뤄보았고 X.509 Certificate을 이용하여 서버, 클라이언트간 인증 방법을 살펴 보았습니다. API 서버도 일반적인 웹 서버와 마찬가지로 TLS mutual authenticate하는 방법에 대해서 직접 실습하였습니다.
다음 포스트에서는 HTTP Authorization Header를 이용하여 쿠버네티스의 인증을 처리하는 방법에 대해서 살펴보도록 하겠습니다.
