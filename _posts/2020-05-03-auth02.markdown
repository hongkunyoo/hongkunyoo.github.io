...
단순 Bearer token을 이용하여 API 서버 인증을 거쳐 보겠습니다. basic auth file과 형식이 유사합니다.
...
```bash
kubectl run cat-token --image k8s.gcr.io/busybox -- cat /var/run/secrets/kubernetes.io/serviceaccount/token
