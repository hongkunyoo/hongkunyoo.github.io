...
Proxy 서버가 API 서버 앞단에 위치하며 LoadBalancer 역할까지 수행할 수도 있습니다. 이 경우 복수의 API 서버에 부하를 분산하여 트래픽을 전달할 수 있습니다.
...
`kubeadm`을 이용하여 쿠버네티스를 구축시, API 서버의 proxy 서버 연결이 이미 설정되어 있습니다. 자세한 값들을 살펴보겠습니다.
